...
/**
 * Class to model a group of cells. Groups may not overlap. If an overlap occurs
 * through expansion, then the groups must merge.
 */
public class Group implements Generational<Group, Void>, Renderable, Countable {
    ...
    public static Group create(long generation, Set<Point> points) {
        if (points.isEmpty()) {
            throw new LifeException("create: points must not be empty");
        }
    ...
    /**
     * Factory method to create a new Group at generation 0, from the given
     * string.
     *
    ...
    public static Group create(long generation, String string) {
        ...
        if (string == null) {
            throw new LifeException("create: was given null string");
        }
    ...
    /**
     * Appends the specified point to the list of points and normalizes the
     * Group.
     *
    ...
    /**
     * Method to add a point expressed in absolute coordinates (relative to
     * Grid).
     *
    ...
    /**
     * Method to add points to this Group where the points are expressed as a
     * String.
     *
     * @param s a comma-separated list of coordinate pairs (separated by a
     * space) with x coming before y.
     ...
     * <p>
     * CONSIDER invoking the add without the normalize, and then do normalize at
     * the end.
     *
    ...
    boolean overlap(Group group) {
        if (extent1 == null || group.extent1 == null) {
            return false;
        }
        if (extent2 == null || group.extent2 == null) {
            return false;
        }
    ...
    Group merge(Group group) throws LifeException {
        if (group == this) {
            throw new LifeException("cannot merge with self");
        }
    ...
    /**
     * Method to create a new version of this Group but translated by x and y
    ...
    /**
     * @return a string representation of the object, with all coordinates in
     * absolute terms.
     */
    ...
    /**
     * Removes the first occurrence of the specified element from cells. Remove
     * does not change the origin or extent but will reduce the count.
     *
    ...
    /**
     * Method to yield the points in this Group with coordinates relative to the
     * Grid.
     *
    ...
    /**
     * Method to yield a String which represents the cells of this Group. Cells
     * are marked '*' unless the cell is at the origin, in which case it is
     * marked 'O'.
     *
     * @return a String.
     */
    ...
    public boolean equals(Object o) {
        ...
        Group group = (Group) o;
        if (points.size() != group.points.size()) {
            return false;
        }
    ...
    private void normalize() {
        ...
        if (origin != null && points.contains(Origin)) {
            return;  // CONSIDER null check of origin may not be necessary
        }
    ...
    /**
     * This method accounts for the change in origin as we go to the new
     * generation.
     *
    ...
    private Point getDiagonal(boolean nw) {
        if (nw) {
            return getAbsolute(extent1.move(0, extent2.getY()));
        } else {
            return getAbsolute(extent1.move(extent2.getX(), 0));
        }
    }

    private void updateExtents(Point point) {
        if (point == null) {
            throw new LifeException("updateExtents: point is null");
        }
        if (extent2 == null) {
            extent2 = point;
        }
        ...
        if (point.getX() >= extent2.getX()) {
            extent2 = new Point(point.getX() + 1, extent2.getY());
        }
        if (point.getY() >= extent2.getY()) {
            extent2 = new Point(extent2.getX(), point.getY() + 1);
        }
        if (point.getX() <= extent1.getX()) {
            extent1 = new Point(point.getX() - 1, extent1.getY());
        }
        if (point.getY() <= extent1.getY()) {
            extent1 = new Point(extent1.getX(), point.getY() - 1);
        }
    }
    ...
    private boolean withinExtents(Point point) {
        if (point == null) {
            throw new LifeException("withinExtents: point is null");
        }
        if (extent2 == null) {
            return false;
        }
    ...
    /**
     * Method to yield this Group's extent1. TODO make private
     *
    ...
    /**
     * Method to yield this Group's extent2. TODO make private
     *
    ...
    public List<Point> getAbsolutePoints() {
        List<Point> pts = new ArrayList<>(getPoints());

        Point origin = this.origin.copy();
        List<Point> result = new ArrayList<>();
        for (Point p : pts) {
     ...
     *
     * @param generation the generation of this Group.
     * @param origin the origin of this Group relative to the origin of the
     * Grid.
     * @param extent1 the extent1 of this Group (i.e. the SW corner of the
     * boundary).
     * @param extent2 the extent2 of this Group (i.e. the NE corner of the
     * boundary).
     * @param points a list of points, which are in the coordinate system of
     * this Group.
     */
     ...
     *
     * @param generation the generation of this Group.
     * @param origin the origin of this Group relative to the Grid (it is
     * required that the origin is one of the points).
     * @param points the points in this Group, with coordinates relative to the
     * origin (points MUST include 0,0).
     */
    ...
    static class CellsAndNeighbors {
         ...
         *
         * @param group the group to which this CellsAndNeighbors object
         * pertains.
         ...
         * @param neighbors the neighbor cells (with width and height extend one
         * in each direction from cells).
         */
        ...
        static CellsAndNeighbors create(Group group) {
            if (group == null) {
                throw new LifeException("CellsAndNeighbors.create: group must not be null");
            }
